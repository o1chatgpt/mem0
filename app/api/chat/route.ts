...
import { getMemoriesFromMem0 } from "@/services/mem0-integration"
...
const FALLBACK_AI_MEMBERS = {
  ...
  mem0: {
    ...
    system_prompt:
      "You are Mem0, a specialized AI assistant with enhanced memory capabilities. Your personality is detail-oriented, personalized, contextual, and adaptive. You excel at remembering user preferences, past conversations, and providing highly personalized assistance. When you recall information from your memories, mention that you remember this from previous interactions to demonstrate your memory capabilities.",
    model: "gpt-4",
  },
}
...
export async function POST(req: Request) {
  try {
    ...
    // Special handling for Mem0 with enhanced memory capabilities
    if (aiFamily === "mem0") {
      try {
        // Use Mem0 integration to get relevant memories
        memories = await getMemoriesFromMem0(
          "00000000-0000-0000-0000-000000000000", // Default user ID
          aiFamily,
          lastUserMessage,
          10, // Get more memories for Mem0
        )
      } catch (error) {
        console.error("Error fetching Mem0 memories:", error)
        // Fall back to regular memory fetch
        memories = await searchMemoriesBySimilarity(aiFamily, lastUserMessage, 5)
      }
    } else {
      // Regular memory retrieval for other AI family members
      try {
        if (lastUserMessage) {
          memories = await searchMemoriesBySimilarity(aiFamily, lastUserMessage, 5)
        }
    ...
    // After processing the message, store it as a memory
    if (messages.length > 0 && lastUserMessage) {
      // Store the last user message as a memory
      try {
        const { error } = await supabase.from("ai_family_member_memories").insert([
          {
            ...
            memory: `User asked: ${lastUserMessage}`,
            embedding: null, // Will be computed by a trigger or background job
          },
        ])

        if (error) {
          console.error("Error storing user message as memory:", error)
        }
      } catch (error) {
