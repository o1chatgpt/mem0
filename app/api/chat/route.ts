...
export async function POST(req: Request) {
  try {
    ...
    try {
      if (lastUserMessage) {
        try {
          // The searchMemoriesBySimilarity function now has its own fallback mechanism
      ...
      // If no memories found, fall back to regular fetch
    ...
    // Use a fallback model if the specified model is not available
    const modelToUse = aiFamilyMember.model || "gpt-4"
    let openAIModel

    try {
      openAIModel = openai(modelToUse)
    } catch (error) {
      console.error(`Error initializing model ${modelToUse}:`, error)
      // Fallback to gpt-3.5-turbo if gpt-4 fails
      openAIModel = openai("gpt-3.5-turbo")
    }
