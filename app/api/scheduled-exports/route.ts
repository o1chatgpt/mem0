...
import { generateExportEmailTemplate, type ExportData } from "@/lib/email-templates/export-template"
import { generateExportErrorEmailTemplate } from "@/lib/email-templates/export-error-template"
import { generatePreviewStats } from "@/lib/email-templates/preview-stats-generator"
    ...
    const results = await Promise.allSettled(
      schedules.map(async (schedule) => {
        try {
          ...
          // Prepare email data
          const aiMemberName = schedule.ai_member_id
            ? schedule.fm_ai_members?.name || `AI Member #${schedule.ai_member_id}`
            : undefined

          const exportData: ExportData = {
            scheduleName: schedule.name,
            exportType: schedule.format as "pdf" | "csv",
            exportDate: new Date().toLocaleString(),
            totalMemories: analyticsData.totalMemories,
            categories: analyticsData.categoriesUsed,
            aiMember: aiMemberName,
            previewStats: generatePreviewStats(analyticsData),
          }

          // Generate email HTML
          const emailHtml = generateExportEmailTemplate(exportData)
          ...
          try {
            const errorData = {
              ...
              nextAttempt: calculateNextScheduledTime(schedule).toLocaleString(),
            }

            const errorEmailHtml = generateExportErrorEmailTemplate(errorData)

            await sendExportEmail(
              schedule.email,
              `Export Error: ${schedule.name}`,
              errorEmailHtml,
              Buffer.from(""),
              "",
              "",
...
async function sendExportEmail(
  ...
  if (attachment.length > 0 && filename && contentType) {
