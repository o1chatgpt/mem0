...
export async function GET() {
  try {
    console.log("Starting CrewAI setup process")

    // First check if the tables already exist
    const tasksTableExists = await tableExists("ai_tasks")
    if (tasksTableExists) {
      console.log("ai_tasks table already exists, skipping creation")
      return NextResponse.json({
        success: true,
        message: "CrewAI tables already exist",
        appliedMigrations: 0,
      })
    }
        ...
        return NextResponse.json(
          {
            ...
            details: "SQL execution failed. Please check database permissions.",
          },
          { status: 500 },
        )
      }
