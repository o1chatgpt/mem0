...
export async function getUserByCredentials(username: string, password: string): Promise<UserSession | null> {
  ...
  // Admin user
  if (
    (username === config.adminUsername || username === config.adminEmail || username === "admin@andiegogiap.com") &&
    (password === config.serverApiKey || password === config.adminPassword)
  ) {
    return {
      id: "admin-1",
      username: username,
  ...
  // Demo user
  if (username === "demo" && password === "demo" && config.enableDemoMode) {
    return {
      id: "demo-1",
      username: "demo",
      role: "user",
