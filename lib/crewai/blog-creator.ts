...
export async function initializeBlogTables() {
  ...
  try {
    ...
    try {
      const { count, error } = await supabase.from("fm_blog_posts").select("*", { count: "exact", head: true })

      tableExists = !error
      console.log("Blog posts table check:", tableExists ? "exists" : "does not exist")
    } catch (error) {
      console.log("Blog posts table does not exist, will create it:", error)
    }
    ...
    if (!tableExists) {
      ...
      if (error) {
        ...
        throw new Error(`Failed to create blog posts table: ${error.message}`)
      }
