...
export interface CrewWorkflow {
  ...
  creator_id: number
  tasks: CrewTask[]
  status: "draft" | "active" | "completed" | "failed" | "paused" | "waiting_approval"
  created_at: string
  updated_at: string
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        created_at: now,
        updated_at: now,
        requires_approval: workflow.requires_approval !== undefined ? workflow.requires_approval : true,
        admin_notes: workflow.admin_notes || null,
      })
      .select()
    ...
    console.log("Workflow created successfully:", data[0])

    // Create tasks for this workflow
    const tasks = []
    if (workflow.tasks && workflow.tasks.length > 0) {
      console.log(`Creating ${workflow.tasks.length} tasks for workflow`)
    ...
    console.error("Detailed error in createWorkflow:", error)
    throw error
  }
}
...
export async function createTask(task: Omit<CrewTask, "id" | "created_at" | "updated_at">): Promise<CrewTask> {
  ...
  try {
    ...
    const taskId = task.id || `task_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`
    const now = new Date().toISOString()
    ...
    console.error("Detailed error in createTask:", error)
    throw error
  }
}
...
export async function getUserWorkflows(userId: number): Promise<CrewWorkflow[]> {
  ...
  try {
    ...
    if (workflowsError) {
      if (workflowsError.message.includes('relation "public.fm_crew_workflows" does not exist')) {
        console.warn("Workflows table does not exist. Returning empty array.")
        return []
      }
