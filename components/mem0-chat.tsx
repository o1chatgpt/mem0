...
export function Mem0Chat({ integration }: Mem0ChatProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      try {
        const addResponse = await fetch("/api/mem0", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (!addResponse.ok) {
          console.warn("Failed to add memory to Mem0, status:", addResponse.status)
        }
      } catch (addError) {
      ...
      // Search for relevant memories with better error handling
      ...
      try {
        ...
        if (!searchResponse.ok) {
          console.warn("Memory search failed, status:", searchResponse.status)
        } else {
          const searchData = await searchResponse.json()
          relevantMemories = searchData.results?.results || []
        }
      } catch (searchError) {
      ...
      if (relevantMemories.length > 0) {
        memoriesContext = "Relevant memories:\n" + relevantMemories.map((m: Memory) => `- ${m.memory}`).join("\n")
      }
      ...
      // Store the AI response in Mem0 as well - with error handling
      try {
      ...
      // Add the AI response even if there was an error
      const errorMessage: Message = {
        role: "assistant",
        content: "I'm having trouble accessing my memory right now, but I'll still try to help you.",
        timestamp: new Date(),
      }
      ...
      toast({
        title: "Memory Issue",
        description: "I encountered an issue with my memory system, but I'll still try to help you.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
