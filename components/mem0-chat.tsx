...
import { BrainCircuit, Send, Info, Tag, Filter, AlertTriangle } from "lucide-react"
...
export function Mem0Chat({ userId, aiMemberId }: Mem0ChatProps) {
  ...
  const [memoryStats, setMemoryStats] = useState<{ count: number; oldestDate: Date | null; newestDate: Date | null }>({
    count: 0,
    oldestDate: null,
    newestDate: null,
  })
    ...
    try {
      ...
      if (data.error === "API_KEY_INVALID") {
        ...
        setMessages((prev) => [
          ...
          {
            role: "system",
            content: "API key configuration error. Please contact your administrator.",
            isError: true,
          },
        ])
      } else if (data.text) {
      ...
      if (error instanceof Error) {
        if (
          ...
          errorMessage =
            "Sorry, the AI model is currently unavailable due to an API key issue. Please contact your administrator."
          setApiKeyError(true)
        }
      }
        ...
        <CardContent className="flex-grow overflow-hidden">
          {apiKeyError && (
            <Alert variant="destructive" className="mb-4">
              ...
              <AlertDescription>
                There is an issue with the OpenAI API key configuration. The chat will continue to work, but AI
                responses will be limited. Please contact your administrator to resolve this issue.
              </AlertDescription>
            </Alert>
          )}
