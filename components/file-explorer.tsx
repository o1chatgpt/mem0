...
export function FileExplorer() {
  ...
  useEffect(() => {
    const checkMemoryEnabled = async () => {
      try {
        const enabled = await mem0Integration.initialize()
  ...
  const loadNavigationHistory = useCallback(async () => {
    ...
    try {
      // Search for navigation memories
      const results = await mem0Integration.searchMemories("Navigated to folder:", "", 20)

      if (results && results.results) {
        // Parse the navigation history from the results
        const history = results.results
          .map((result: any) => {
            const match = result.memory.match(/Navigated to folder: (.+)/)
            if (match && match[1]) {
              return {
                path: match[1],
  ...
  const loadFileTags = useCallback(async () => {
    ...
    try {
      ...
      for (const file of files) {
        const tags = await memoryStore.getFileTags(file.id)
        if (tags && tags.length > 0) {
          tagsMap[file.id] = tags
        }
      }
  ...
  const loadRecommendations = useCallback(async () => {
    ...
    try {
      // Get user activity summary from mem0
      const activity = await mem0Integration.getUserActivitySummary("")

      // Use the activity data to generate recommendations
      // This is a simplified implementation - in a real app, you'd have more sophisticated logic
      const recentPaths = activity.recentFolders
        .map((folder: any) => {
          const match = folder.memory.match(/Navigated to folder: (.+)/)
          return match && match[1] ? match[1] : null
        })
        .filter(Boolean)

      // Find files that are in recently visited folders but not in the current path
      const recommendedFiles = recentFiles.filter((file) => {
        const fileFolder = file.path.substring(0, file.path.lastIndexOf("/"))
        return recentPaths.includes(fileFolder) && fileFolder !== currentPath
      })
  ...
  }, [memoryEnabled, recentFiles, currentPath])
  ...
  useEffect(() => {
    const recordNavigation = async () => {
      ...
      try {
        await mem0Integration.addNavigationMemory(currentPath, "")
        // Refresh navigation history after recording
        await loadNavigationHistory()
      } catch (error) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      ...
      // Record search in mem0
      if (memoryEnabled) {
        try {
          await mem0Integration.addSearchMemory(searchQuery, "", filteredFiles.length)
        } catch (error) {
  ...
  const addTag = async (fileId: string, tag: string) => {
    try {
      ...
      if (memoryEnabled) {
        ...
        if (file) {
          await mem0Integration.addMemory(`Added tag "${tag}" to file "${file.name}"`, "")
        }
      }
    } catch (error) {
  ...
  const removeTag = async (fileId: string, tag: string) => {
    try {
      ...
      if (memoryEnabled) {
        const file = files.find((f) => f.id === fileId)
        if (file) {
          await mem0Integration.addMemory(`Removed tag "${tag}" from file "${file.name}"`, "")
        }
      }
    } catch (error) {
