...
import { getTemplatesForLanguage, type FileTemplate } from "@/lib/file-templates"
...
export function TabbedEditor({ files, setFiles }: TabbedEditorProps) {
  ...
  const [availableTemplates, setAvailableTemplates] = useState<FileTemplate[]>([])
  ...
  // Update available templates when filename changes
  useEffect(() => {
    if (newFileName) {
      const language = getFileLanguage(newFileName)
      const templates = getTemplatesForLanguage(language)
      setAvailableTemplates(templates)
      setSelectedTemplate(templates.length > 0 ? templates[0].name : null)
    } else {
  ...
  const createNewFile = () => {
    ...
    toast({
      title: "File created",
      description: `Created ${newFileName} with ${selectedTemplate || "empty"} template`,
      duration: 2000,
    })
  }
              ...
              <div className="flex flex-col gap-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && !availableTemplates.length) createNewFile()
                    }}
                  />
                  <p className="text-sm text-muted-foreground">Supported extensions: .md, .html, .css, .js</p>
                </div>
