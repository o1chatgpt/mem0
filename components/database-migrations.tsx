  ...
  const fetchMigrationStatus = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.error("Error response from migration status API:", data)
      }
    } catch (error) {
      console.error("Error fetching migration status:", error)
      setError("Failed to fetch migration status. Check the console for details.")
      toast({
        title: "Error",
      ...
      <Card>
        ...
        <CardContent>
          ...
          <ul className="list-disc pl-5 space-y-2 mb-4">
            <li>The migrations table does not exist yet</li>
            <li>The database connection is not properly configured</li>
            <li>
              The <code>exec_sql</code> RPC function is not available in your Supabase instance
            </li>
          </ul>

          <p className="mb-4">To fix this, you may need to:</p>
          <ul className="list-disc pl-5 space-y-2">
            <li>
              Create the <code>schema_migrations</code> table manually
            </li>
            <li>
              Create the <code>exec_sql</code> RPC function in your Supabase instance
            </li>
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>No migration data available</CardDescription>
        </CardHeader>
        <CardContent>
          <p>Unable to retrieve migration status information.</p>
        </CardContent>
        <CardFooter>
          <Button onClick={fetchMigrationStatus}>Retry</Button>
        </CardFooter>
